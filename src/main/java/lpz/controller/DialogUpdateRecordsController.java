...
public class DialogUpdateRecordsController implements Initializable {
    ...
    public DialogUpdateRecordsController(String selectedTable, String storeAddress, String bookName, int value) {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ArrayList<String> bookList = clientPostgreSQL.stringListQuery("SELECT название FROM книги ORDER BY id", "название");
        ArrayList<String> storeList = clientPostgreSQL.stringListQuery("SELECT адрес FROM магазины ORDER BY id", "адрес");

        String address = clientPostgreSQL.getInfoByColoumnQuery("SELECT адрес FROM магазины WHERE адрес=?", storeAddress);
        String book = clientPostgreSQL.getInfoByColoumnQuery("SELECT название FROM книги WHERE название=?", bookName);
...
//            System.out.println("if-else: CHECK");
            ...
            try (Connection connection = clientPostgreSQL.getConnection();
                 PreparedStatement preparedStatement = connection.prepareStatement(
                         "SELECT количество_книг FROM учет WHERE название_книги = ? AND адрес_магазина = ?")) {

                bookName = bookNameChoice.getValue();
                storeAddress = storeAdressChoice.getValue();
...
//                System.out.println(preparedStatement);
                ...
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    if (resultSet.next()) {
                        int bookCount = resultSet.getInt("количество_книг");
    ...
    private void onActionBottomAdd(ActionEvent event) {
        ...
        if (!quantityText.isEmpty() && bookNameChoice.getValue() != null && storeAdressChoice.getValue() != null) {
            try {
                ...
                if (newValue < 0) {
                    MyAlerts.showInfoAlert("Недостаточно книг в магазине");
//                    showInfoAlert("Недостаточно книг в магазине");
//                    new Alert(Alert.AlertType.WARNING, "Недостаточно книг в магазине").showAndWait();
                } else {
                    if (!newRecord) {
                        String updateQuery = "UPDATE учет SET количество_книг = " + newValue + " WHERE название_книги = '" + bookName + "' AND адрес_магазина = '" + storeAddress + "'";
//                    System.out.println("количество_книг = " + newValue + " WHERE название_книги = '" + bookName + "' AND адрес_магазина = '" + storeAddress + "'");
                        ...
                        String addQueryTemplate = "INSERT INTO учет (название_книги, адрес_магазина, количество_книг) VALUES ('%s', '%s', %s)";
